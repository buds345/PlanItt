...
from auth.email_utils import send_verification_emailguest
...
class AuthApp(ctk.CTk):
    ...
    def show_add_guest_ui(self, event_id):
        self.clear_ui()
        ctk.CTkLabel(self, text="Add Guest", font=("Segoe UI", 22)).pack(pady=20)

        self.guest_name_entry = ctk.CTkEntry(self, placeholder_text="Guest Name")
        self.guest_name_entry.pack(pady=10)

        self.guest_email_entry = ctk.CTkEntry(self, placeholder_text="Guest Email")
        self.guest_email_entry.pack(pady=10)

        ctk.CTkButton(self, text="Add Guest", command=lambda: self.add_guest(event_id), fg_color="#ff007f").pack(pady=20)
        ctk.CTkButton(self, text="Back to Dashboard", command=lambda: self.show_event_dashboard(event_id), fg_color="transparent", border_color="#ff007f", border_width=2, text_color="#ff007f").pack()
    ...
    def add_guest(self, event_id):
        ...
        if not guest_name or not guest_email:
            messagebox.showerror("Error", "Please fill in both Name and Email fields.")
            return

        guest = Guest(name=guest_name, email=guest_email, event_id=event_id)
        guest.invite()

        messagebox.showinfo("Success", f"Invitation sent to {guest_name}!")
        self.guest_name_entry.delete(0, ctk.END)
        self.guest_email_entry.delete(0, ctk.END)
